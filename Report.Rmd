...
# 1. Connect  R studio to the pgadmin test database named "Final Database".
```{r}
require('RPostgreSQL')
require('DBI')
...
con<- dbConnect(drv, dbname = 'Finalized Dataset',
                host = 's19db.apan5310.com', port = 50106,
                user = 'postgres', password = '2nfupiey')
```
...
# 3. Import dataset and randomly poick 600,000 records.
```{r}
table <- read.csv('Parking_Violations_Issued_-_Fiscal_Year_2017.csv')
head(table)
nrow(table)
ncol(table)
table<-table[sample(nrow(table), 600000), ]
```

# 4. Change all the names to the lower case for furthar exploration.
```{r}
...
#insert the vehicle values into the table
table$vehicle<-vehicle_table$vehicle
#Extract unique vehicle value from the table
vehicle_code_table <- data.frame('vehicle' = unique(table$vehicle))
#Generate the primary key with the unique value
...
#Connect the database and transform the data
...
# 17. Import the values into pgadmin database
```{r}
